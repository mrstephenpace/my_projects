...
class BBoxVisualization():
    ...
    def draw_bboxes(self, img, box, conf, cls):
        ...
        for bb, cf, cl in zip(box, conf, cls):
            ...
            avg_width = 24.0
            dist = distance_to_camera(avg_width, focalLength, int(x_max-x_min))/12

            # display distance of hand to camera
            if dist < 5:
              dist_font_color = (0,0,255)
            else:
              dist_font_color = (255,0,0)
            cv2.putText(img, 'person {0:.0f}'.format(i+1)+' distance: '+str("{0:.2f}".format(dist)+' feet'),
            (int(img_w*0.5),int(img_h*0.1+30*i)),cv2.FONT_HERSHEY_PLAIN, 1, dist_font_color, 2)
